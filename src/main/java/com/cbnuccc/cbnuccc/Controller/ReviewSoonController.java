...
public class ReviewSoonController {
    ...
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "전체 점검순 목록 조회 성공",
                content = @Content(examples = @ExampleObject(
                    ...
                    value = "{\"data\":[{\"id\":1,\"name\":\"1점검순\"},{\"id\":2,\"name\":\"2점검순\"}],\"length\":2,\"pageAt\":0,\"totalPage\":1,\"totalElement\":2}"
                ))),
        @ApiResponse(responseCode = "401", description = "인증 실패 (토큰 미제공 또는 만료)", content = @Content)
    })
    @GetMapping("/review-soon")
    public ResponseEntity<?> getAllReviewSoons(@ParameterObject Pageable pageable) {
        Page<ReviewSoonInfoDto> result = reviewSoonService.getAllReviewSoons(pageable);
    ...
    public ResponseEntity<?> getReviewSoonById(
        ...
        DataWithStatusCode<ReviewSoonInfoDto> result = reviewSoonService.getReviewSoonById(id);
